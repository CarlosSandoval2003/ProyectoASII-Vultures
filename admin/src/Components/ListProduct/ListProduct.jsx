...
const ListProduct = () => {
  ...
  const fetchProducts = async () => {
    try {
      let url = 'https://plankton-app-nhztk.ondigitalocean.app/allproducts';
      // Si hay un término de búsqueda, agregarlo a la URL
      if (searchTerm.trim() !== '') {
        url = `https://plankton-app-nhztk.ondigitalocean.app/searchproduct?search=${encodeURIComponent(searchTerm)}`;
      }
  ...
  const fetchItemProducts = async (productId) => {
    try {
      const response = await fetch(`https://plankton-app-nhztk.ondigitalocean.app/itemproducts/${productId}`);
      const data = await response.json();
  ...
  const updateItemProducts = async (item) => {
    try {
      ...
      await fetch("https://plankton-app-nhztk.ondigitalocean.app/updateitemproduct", {
        method: "POST",
        headers: {
  ...
  const removeProduct = async (id) => {
    try {
      await fetch('https://plankton-app-nhztk.ondigitalocean.app/removeproduct', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
const handleEdit = async (product) => {
  ...
  try {
    ...
    const response = await fetch(`https://plankton-app-nhztk.ondigitalocean.app/itemproducts/${product.id}`);
    const data = await response.json();
    setItemProducts(data);

    // Obtener los títulos de los ITEM_PRODUCTO
    const titlesResponse = await fetch(`https://plankton-app-nhztk.ondigitalocean.app/itemproducttitles/${product.id}`);
