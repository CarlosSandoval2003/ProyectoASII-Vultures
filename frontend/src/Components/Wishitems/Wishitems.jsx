...
const Wishitems = () => {
  ...
  useEffect(() => {
    if (userId && userId !== "0") {
      fetch(`https://plankton-app-nhztk.ondigitalocean.app/wishlistItems/${userId}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleRemoveItem = async (userId, itemId) => {
    try {
      const response = await fetch("https://plankton-app-nhztk.ondigitalocean.app/removeWishlistItem", {
        method: "DELETE",
        headers: {
  ...
  const handleAddToCart = async (userId, wishlistItemId) => {
    try {
      ...
      const wishlistResponse = await fetch(`https://plankton-app-nhztk.ondigitalocean.app/wishlistItem/${wishlistItemId}`);
      ...
      if (cartData.cartExists) {
        ...
        const createCartResponse = await fetch("https://plankton-app-nhztk.ondigitalocean.app/cart", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const addToCartResponse = await fetch(`https://plankton-app-nhztk.ondigitalocean.app/cartItem/${cartId}/${itemProductId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (addToCartData.success) {
        ...
        const removeFromWishlistResponse = await fetch("https://plankton-app-nhztk.ondigitalocean.app/removeWishlistItem", {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
